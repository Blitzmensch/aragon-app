...
| **Development** | https://dev.app.aragon.org     | GKE     | Automatic on `develop` branch push or manual by the `WebApp Deploy DEV` workflow    | [Depends on the selected branch] |
| **Testing**     | https://test.app.aragon.org    | GKE     | Automatic on `DOPS-xyz` branches push or manual by the `WebApp Deploy DEV` workflow | `.env.development`               |
