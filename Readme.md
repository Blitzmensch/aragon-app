...
| Name            | Url                            | Hosting | Deployment trigger                                                                  | Environment file                 |
| --------------- | ------------------------------ | --------------------------------------------------------------------------------              | -------------------------------- |
| **Local**       | http://localhost:3000          | -       |                                                                                     | `.env.development`               |
| **Preview**     | [Generated by the pipelines]   | Fleek   | Manual by adding the `preview` label to an open Pull Request                        | `.env.staging`                   |
| **Development** | https://app-dev.aragon.org     | GKE     | Automatic on `develop` branch push or manual by the `WebApp Deploy DEV` workflow    | [Depends on the selected branch] |
| **Testing**     | https://app-test.aragon.org    | GKE     | Automatic on `DOPS-xyz` branches push or manual by the `WebApp Deploy DEV` workflow | `.env.development`               |
| **Staging**     | https://staging-app.aragon.org | Fleek   | Automatic on `develop` branch push                                                  | `.env.staging`                   |
| **Production**  | https://app.aragon.org         | Fleek   | Automatic on `main` branch push                                                     | `.env.production`                |
